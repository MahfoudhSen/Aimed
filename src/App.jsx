  ...
  async function doSearch() {
    ...
    try {
      const res = await fetch(
        `${API}/api/search-analyze?borough=${encodeURIComponent(borough)}&budget=${max}&room_type=${encodeURIComponent(type)}`
      );
      await stepTimer;
      setStep(5);
      ...
      setResults({
        listings,
        safe: listings.filter(l => l.safe).length,
        scams: listings.filter(l => !l.safe).length,
      });
      setPhase("results");
    } catch (err) {
      await stepTimer;
      ...
      <div style={{ maxWidth: 620, margin: "0 auto", padding: "32px 20px 60px" }}>
        ...
        {phase === "error" && (
          <div style={{ textAlign: "center", paddingTop: 60 }}>
            ...
            <h2 style={{ ...s, fontWeight: 800, fontSize: 20, color: R, marginBottom: 8 }}>Search failed</h2>
            <p style={{ color: "#555", fontSize: 13, marginBottom: 24 }}>{error || "Could not reach the server. Make sure the backend is running."}</p>
            <button onClick={() => setPhase("select")} style={{ background: "#141414", border: "1px solid rgba(255,255,255,0.1)", color: "#888", padding: "10px 20px", borderRadius: 10, cursor: "pointer", fontSize: 13 }}>← Try again</button>
          </div>
        )}
          ...
          <>
            ...
            {results.listings.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 20px", color: "#444" }}>
                ...
                <p>No listings found. Try a different borough or budget.</p>
              </div>
            ) : (
              <>
